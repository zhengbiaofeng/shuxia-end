...
- Source control is split into three independent GitHub repositories: `shuxia-end` for the administration frontend, `shuxia-reader` for the reader frontend, and `shuxia-server` for the backend. The reader now has its own Git history and no longer depends on the legacy parent Gitee repository.
...
- Comic management now supports multi-image upload and scans only enabled, writable local/NAS locations whose Storage Management scope is `comic`. Uploads and scans create comic, episode, page-file and optional cover-file relations without falling back to book, novel or audio storage.
- Comic scan treats a directory containing images as one comic and its immediate child directories as episodes. Covers use the names `cover`, `folder`, `front` or `封面`; page files use natural ordering; repeated scans skip already-related normalized paths.
...
- fnOS release package verification passed at `E:\code\trae_workspcae\shuxia\qianduan\shuxia-end\output\fnos-release-0.3.0-final`: three amd64 image archives, Compose version pins, migration order, SHA256 manifest and no-user-data checks passed; package metadata reports `productionReady=true`.
