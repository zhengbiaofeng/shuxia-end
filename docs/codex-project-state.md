...
- `连接模板配置` has a top action `返回扫描源` that routes back to `/automation/rules`, so the template page is clearly a child of scan-source management; its create action is labeled `新增连接模板` instead of `添加渠道`.
